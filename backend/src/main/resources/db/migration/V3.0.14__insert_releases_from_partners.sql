...
-- 각 파트너의 사용자를 요청자로 하여 다양한 릴리즈 데이터 생성 (총 85개)

-- ========================================
-- REGULAR 릴리즈 - DEPLOYED (정기 배포 완료) (30개)
-- ========================================

-- 삼성그룹 정기 배포 완료 릴리즈 (8개)
...
SELECT 
    ...
    CASE (row_number() OVER ()) % 8
        ...
        ELSE 'CRM 고객 세분화 및 마케팅 자동화. 주요 변경사항: RFM 분석 기능 추가, 자동 캠페인 발송'
    END as content,
...
-- 현대그룹 정기 배포 완료 릴리즈 (7개)
INSERT INTO releases (release_number, title, release_type, status, content, requester_id, requester_dept_id, approver_id, scheduled_at, deployed_at, created_by, updated_by)
SELECT 
    'REL2501-' || LPAD((9 + (row_number() OVER () - 1))::TEXT, 4, '0') as release_number,
    CASE (row_number() OVER ()) % 7
        ...
        ELSE '정비 이력 관리 시스템 v1.6.1 정기 배포'
    END as title,
    'REGULAR' as release_type,
    'DEPLOYED' as status,
    CASE (row_number() OVER ()) % 7
        ...
        ELSE '차량 정비 이력 통합 관리. 주요 변경사항: 정비 패턴 분석, 부품 교체 주기 예측, 고객 만족도 조사 자동화'
    END as content,
    u.id as requester_id,
    dept.id as requester_dept_id,
    approver.id as approver_id,
...
-- SK그룹 정기 배포 완료 릴리즈 (7개)
INSERT INTO releases (release_number, title, release_type, status, content, requester_id, requester_dept_id, approver_id, scheduled_at, deployed_at, created_by, updated_by)
SELECT 
    'REL2501-' || LPAD((16 + (row_number() OVER () - 1))::TEXT, 4, '0') as release_number,
    CASE (row_number() OVER ()) % 7
        ...
        ELSE '물류 센터 자동화 시스템 v1.8.0 정기 배포'
    END as title,
    'REGULAR' as release_type,
    'DEPLOYED' as status,
    CASE (row_number() OVER ()) % 7
        ...
        ELSE '물류 센터 로봇 자동화 확대. 주요 변경사항: AGV 경로 최적화, 재고 실시간 추적, 피킹 효율 30% 향상'
    END as content,
    u.id as requester_id,
    dept.id as requester_dept_id,
    approver.id as approver_id,
...
-- LG그룹 정기 배포 완료 릴리즈 (8개)
INSERT INTO releases (release_number, title, release_type, status, content, requester_id, requester_dept_id, approver_id, scheduled_at, deployed_at, created_by, updated_by)
SELECT 
    'REL2501-' || LPAD((23 + (row_number() OVER () - 1))::TEXT, 4, '0') as release_number,
    CASE (row_number() OVER ()) % 8
        ...
        ELSE '클라우드 인프라 관리 시스템 v4.2.0 정기 배포'
    END as title,
    'REGULAR' as release_type,
    'DEPLOYED' as status,
    CASE (row_number() OVER ()) % 8
        ...
        ELSE '클라우드 리소스 자동 스케일링. 주요 변경사항: 부하 기반 자동 확장, 비용 최적화, 장애 자동 복구'
    END as content,
    u.id as requester_id,
    dept.id as requester_dept_id,
    approver.id as approver_id,
...
INSERT INTO releases (release_number, title, release_type, status, content, requester_id, requester_dept_id, approver_id, scheduled_at, deployed_at, created_by, updated_by)
SELECT 
    'REL2501-' || LPAD((39 + (row_number() OVER () - 1))::TEXT, 4, '0') as release_number,
    CASE (row_number() OVER ()) % 8
...
INSERT INTO releases (release_number, title, release_type, status, content, requester_id, requester_dept_id, approver_id, scheduled_at, deployed_at, created_by, updated_by)
SELECT 
    'REL2501-' || LPAD((47 + (row_number() OVER () - 1))::TEXT, 4, '0') as release_number,
...
INSERT INTO releases (release_number, title, release_type, status, content, requester_id, requester_dept_id, approver_id, scheduled_at, deployed_at, created_by, updated_by)
SELECT 
    'REL2501-' || LPAD((56 + (row_number() OVER () - 1))::TEXT, 4, '0') as release_number,
...
INSERT INTO releases (release_number, title, release_type, status, content, requester_id, requester_dept_id, approver_id, scheduled_at, deployed_at, created_by, updated_by)
SELECT 
    'REL2501-' || LPAD((76 + (row_number() OVER () - 1))::TEXT, 4, '0') as release_number,
...
-- V3.0.8__partner_migration.sql에 있는 파트너 기준으로 총 85개의 RELEASE 데이터를 생성하였습니다.
...
-- 
-- 릴리즈 구성:
--   - REGULAR + DEPLOYED: 30개 (삼성 8 + 현대 7 + SK 7 + LG 8)
--   - REGULAR + APPROVED: 25개 (롯데 8 + GS 8 + IT기업 9)
--   - EMERGENCY + DEPLOYED: 20개
--   - REGULAR + REQUESTED: 10개
--   - 총 85개의 다양한 릴리즈로 실제 배포 관리 환경을 시뮬레이션
--   - 릴리즈 번호: REL2501-0001 ~ REL2501-0085
